...
public abstract class Shell {
	...
	public Shell(String n, Writer w, boolean flush) {
		...
		/*
		 * new Command(this, "exec", "execute an external command") { void
		 * execute(String[] args) { try { Sys.output(out); Sys.main(args); }
		 * catch (Exception e) { throw new RuntimeException(""+e); } } }; new
		 * Command(this, "run", "run a class method") { void execute(String[]
		 * args) { try { if (args.length > 0) shell().run(args[0],
		 * Arrays.copyOfRange(args, 1, args.length)); else throw new Exception(
		 * "Nothing to run!"); } catch (Exception e) { throw new
		 * RuntimeException(""+e); } } };
		 */
